...
async def get_cpu_metrics(
    ...
    Args:
        ...
        period:      Metric granularity in seconds (default 300 = 5 min).
        minutes:     How far back to look (default 60 = 1 hour).

    Returns:
        Dict with ``instance_id``, ``datapoints`` (timestamp, average,
...
async def get_cpu_metrics_for_instances(
    ...
    Args:
        ...
        period:       Metric granularity in seconds.
        minutes:      Lookback window.

    Returns:
        Dict mapping each instance_id to its metrics result.
    """
...
async def get_memory_metrics(
    ...
    Returns:
        Dict with datapoints and summary, or a stub message if the
...
async def get_disk_usage(
    ...
    Args:
        instance_id: The EC2 instance ID.
        mount_path:  Filesystem mount point (default ``/``).
        period:      Metric granularity in seconds.
        minutes:     Lookback window.
